...
| 4 | iPad 首页更多列表与平台 Tab | 新包更多列表无卡片重叠；冷启动与侧栏切换正常；同名插件、元数据更新、移除选中插件时身份和选择稳定 | **平台 Tab 本轮验证通过，未复现崩溃（9 月 15 日更新）**：按用户确认移出本轮待验收清单，实际覆盖范围见[新包记录](BugsnagBuild19Fixes20260915.md#ipad-tabbar)；首页更多列表仍保留原验收状态 |
...
| 8 | iOS App 在 Mac 上运行 | 该运行模式下检查冷启动、历史进入播放、封面导航、退出重进和窗口缩放；异常时采集可定位的主线程证据 | **本轮已覆盖场景通过，未复现原问题（9 月 16 日用户确认）**：非空历史页、收藏实际起播及返回、标签往返、窗口放大和恢复通过，未复现模型缺失或布局挂起；历史记录实际起播、长时间播放等未覆盖说明保留，见[追加记录](BugsnagBuild19Fixes20260915.md) |
