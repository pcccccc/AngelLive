...
> 状态：实施中
> 进度：Phase 1–4 自动化与模拟器回归已完成；tvOS 实体遥控器和剩余系统级场景待真机验收
> 日期：2026-07-14
...
- iOS 真机已由用户确认自动 PiP 与后台播放设置可用；后台音频、PiP 创建/启动和前台停止链路均保留。
  来电/音频中断恢复、控制中心与锁屏远控仍需单独做系统级验收，不能由构建结果代替。
- macOS workspace Debug 构建通过。运行态同时打开两个真实直播窗口，切换 key window 后关闭原 owner，
  剩余窗口自动成为 key 并持续播放。`RemoteControlOwnerTests` 验证 inactive layer 的 `stop()` 不会移除
  active owner 的 command targets；测试关闭自动起播以隔离真实网络/解码副作用。
- tvOS workspace Debug 使用正常的 `Sign to Run Locally` 构建、安装并持续运行。Apple TV 4K 模拟器已验证
  顶部导航的 Focus 移动、Select 进入配置页、配置表单 Focus 下移和系统键盘弹出。首次禁用代码签名的产物因
  缺失 CloudKit entitlement 在 `CKContainer` 初始化处 `SIGTRAP`，恢复本地签名后消失，属于验证环境问题。
- tvOS 播放页的 Play/Pause、两级 Menu/Exit、统计/清晰度面板返回路径已完成静态接线审计，但本次模拟器
  没有可直接复用的收藏播放入口；Siri Remote 触控环手感、系统音量 HUD 和真机性能仍必须在实体 Apple TV 验证。

本轮自动回归：AngelLiveCore 55 项测试（12 个 suite）、KSPlayer `swift build`、异步 track 生命周期测试、
Remote Command owner 定向测试，以及 iOS/macOS/tvOS Debug 构建均通过。KSPlayer 全量测试仍仅受既有
`FFmpegUtilityTests.conversionAudio` 旧 `mediaType:` 参数编译基线阻塞。
