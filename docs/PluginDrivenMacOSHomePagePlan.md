...
- iPad 的纵向 `ScrollView` + `LazyVStack` 和全宽 Hero 作为结构基线；Mac 房间分区改用随窗口宽度换行的纵向网格，避免嵌套横向滚动。
...
- 宽窗口采用类 Twitch 的居中焦点模块：左侧严格 16:9 媒体与右侧信息面板无缝组成一张卡片，只裁切整个模块的四个外角，统一使用 16 pt 圆角；整体约占内容区宽度的 78%、最宽 1080 pt，不随窗口无限拉伸。窄窗口回退为单个 16:9 圆角图文叠加卡片。
- macOS 焦点位优先使用插件专门提供的 Banner `imageURL`，房间 `roomCover` 仅作为兜底；按媒体实际显示尺寸和 Retina scale 下采样，不把普通封面放大到整个内容区。
- 宽窗口的标题、可选副标题、Badge 和来源放在右侧信息面板；空值和 `-` 不生成占位文案，缺失内容由后续信息自然补位。主播名、标题、副标题和 Badge 做规范化去重，不重复展示同一文本。底部使用平台自身图标与名称标记来源，并显示明确的观看/分类动作；窄窗口沿用相同数据规则，只把信息移到底部渐变遮罩内。
- Banner 不创建横向滚动容器，使用左右按钮与自动轮播切换，从结构上避免 macOS 系统横向 scroller；不注册普通鼠标滚轮监听，也不拦截首页纵向滚动事件。
- 索引使用自动轮播进度胶囊并完整展示全部 Banner 项；隐藏系统横向滚动条，不用 scroller 代替索引。
...
- 分区使用 `LazyVGrid` 自适应换行，首页只保留外层纵向滚动，不再嵌套横向滚动区域。
- 房间卡片复用 macOS `LiveRoomCard`，宽度在 196–248 pt 范围随窗口自适应；同一分区等宽等高。
...
- 分区右侧统一显示“全部”：有 `seeAllTarget` 时进入共享分页模型驱动的 Mac adaptive grid；
  没有目标时展示本次推荐结果的本地完整网格，不伪造分类参数，也不额外调用插件。
...
- Banner 左右按钮和自动轮播；普通鼠标滚轮与触控板纵向手势只保留页面纵向滚动语义。
- 首页收藏和推荐分区均随窗口宽度换行，鼠标滚轮只驱动外层纵向页面。
