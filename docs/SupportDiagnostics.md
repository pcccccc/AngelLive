...
- 应用版本、系统、设备型号、录制起止时间，以及开始记录时的时区、语言与后台播放设置。
- 页面切换、搜索、进入直播间、重试、清晰度或线路切换、前后台变化，以及账号和插件管理入口。
- 插件标识、版本、调用方法、开始时间、耗时、结果和操作关联 ID。
- JavaScript 异常的消息、堆栈、脚本位置（运行时能够提供时）。
- HTTP 方法、实际请求 URL、状态码、耗时、请求与响应头、原始文本正文、失败原因，以及正文类型、原始字节数和截断标记。HTML 保留 HTML 原文，JSON 保留字段顺序、空白、数值表示及转义形式；不解析重排或用摘要代替正文。插件调用参数、插件处理后的返回，与 HTTP 原始响应分别展示。
- 进入和离开直播间、重试及播放选择携带公开房间 ID、主播、标题、运行时平台标识和线路／清晰度名称；实际插件版本来自调用记录，不把磁盘最高安装版本当作运行版本。

操作时间线描述应用中的业务操作，不采集逐次触摸、键盘事件或屏幕录像。关键词搜索记录实际输入（凭证字段脱敏），分享搜索在时间线中记录解析后的公开房间信息。业务调用参数仍可能包含原始分享文本与搜索词，分享前需要检查。播放器内部及原生流解析器未经过插件 HTTP bridge 的流量不在 HTTP 记录范围内。弹幕帧与定时调用仅记录协议摘要，不保存原始二进制帧和用户弹幕内容。

业务调用、播放器日志、弹幕高频调用采用独立诊断缓冲，分别最多保留 200、50、30 条；开发者控制台的 500 条环不再决定诊断报告的寿命。高频解码日志不能淘汰进入房间时的参数、起播请求或失败调用。报告先给出复现操作、缺失信息和失败索引，再提供完整正文；底层错误日志不自动等同于用户现象或问题根因。
...
常见凭证字段、Cookie、Authorization、URL 中的凭证值在进入诊断内容时脱敏；房间 ID、分页等普通查询参数保留。脱敏只替换敏感值所在片段，不重新序列化整个 HTML／JSON。显式诊断时，普通业务请求不因为携带标准 Cookie／Authorization 就省略全部响应；登录事务、敏感运行时调用以及无法安全分离的私有凭证注入继续省略正文，并明确标记。二进制响应只记录类型和大小，不转为可分享的原始数据。

单段正文最多 1 MiB，运行中的诊断正文总量最多 32 MiB，报告 JSON 最多 16 MiB；达到总量限制时优先缩短高频日志和最大响应，尽量保留请求参数、调用标识、状态及关联，并在正文和报告前部标明截断与原始字节数。未触及限制的 HTML／JSON 原文须通过录制、保存、恢复和导出的逐字一致测试。旧版报告兼容读取，但其中已经遗漏或重排的数据不能恢复。
...
## 2026-09-18 原始正文与复现信息修复

HTTP 采集改为保存实际 URLRequest 的参数和收到的 UTF-8 正文；原始 HTML／JSON 与插件处理后的返回分别保存。脱敏只修改敏感片段，普通字段的顺序、空白、数值表示和转义不再被重写。原文记录只在显式诊断会话中启用，非录制时的开发者控制台保留既有输出方式。

三端的进房、搜索、重试与播放选择补齐公开上下文及操作关联；iOS 切房和三端异步刷新沿用发起操作的关联。业务调用与播放器／弹幕高频日志分区保留，淘汰后的晚到正文不能通过开发者控制台副本重新进入活动诊断的内存预算。

- 最终 Core 全量回归：`swift test --package-path Shared/AngelLiveCore --no-parallel`，44 个套件、328 项通过。证据：`/private/tmp/angellive-raw-diagnostics-core-full.log`。
- 回归覆盖原始 HTML／JSON 逐字保存、恢复和导出，凭证脱敏、普通认证请求的响应保留、日志洪泛、首尾与失败证据保留、晚到回调、停止冻结、跨会话隔离和正文预算。
- 全仓扫描覆盖 669 个跟踪或未忽略文件，其中 586 个 UTF-8 文本、83 个二进制或非 UTF-8 文件；4 行通用变量／查询占位及历史扫描说明的歧义候选已逐项复核，无实际内容平台标识，无测试数值映射候选。二进制未做 OCR；`git diff --check` 通过。
- 本轮未安装或运行设备新包，真实直播录制和三端设备交互未验证；不引用此前界面任务的旧包或截图作为本轮证据。
